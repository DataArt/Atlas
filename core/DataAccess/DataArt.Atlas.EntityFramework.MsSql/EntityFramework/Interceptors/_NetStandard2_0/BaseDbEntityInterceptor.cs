...
namespace DataArt.Atlas.EntityFramework.MsSql.EntityFramework.Interceptors
{
    public abstract class BaseDbEntityInterceptor : IDbInterceptor
    {
        ...
        protected virtual bool IsTargetEntity(EntityEntry item, EntityState state)
        {
            return TargetType.IsInstanceOfType(item.Entity);
        }

        protected virtual void OnBefore(EntityEntry item, EntityState state, DbInterceptionContext context)
        {
        }

        protected virtual void OnAfter(EntityEntry item, EntityState state, DbInterceptionContext context)
        {
        }
    }
}
